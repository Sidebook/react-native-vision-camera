...
if (CameraModule == null)
    console.error("Camera: Native Module 'CameraView' was null! Did you run pod install?");
...
export class Camera extends React.PureComponent {
    ...
    async getAvailableVideoCodecs(fileType) {
        if (Platform.OS !== 'ios')
            return []; // no video codecs supported on other platforms.
        try {
    ...
    render() {
        ...
        return (<NativeCameraView {...props} frameProcessorFps={frameProcessorFps === 'auto' ? -1 : frameProcessorFps} cameraId={device.id} ref={this.ref} onViewReady={this.onViewReady} onInitialized={this.onInitialized} onRecordingStart={this.onRecordingStart} onError={this.onError} onFrameProcessorPerformanceSuggestionAvailable={this.onFrameProcessorPerformanceSuggestionAvailable} enableFrameProcessor={frameProcessor != null}/>);
    }
}
//#endregion
